...
const main =async ()=>{
  const transport = new StdioImporterTransport({
      command: "node",
      args: ["./dist/examples/example1/server.js"]
    });

  const importer = new Importer(); 
  await importer.import(transport);
  // or
  // await importer.connect(transport);
  
  // const functions = await importer.listFunctions();
  // const result = await importer.callFunction({name:"greeting", arguments:["jerin",2] })
  // console.log(result)
