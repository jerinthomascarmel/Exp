...
from exp.exporter.highlevel.exceptions import InvalidSignature
from exp.exporter.highlevel.utilities.logging import get_logger
from exp.exporter.highlevel.utilities.types import Audio, Image
from exp.types import CallFunctionResult, ContentBlock, TextContent
...
class FuncMetadata(BaseModel):
    ...
    output_model: Annotated[type[BaseModel],
                            WithJsonSchema(None)] | None = None
    ...
    def convert_result(self, result: Any) -> Any:
        ...
        Convert the result of a function call to the appropriate format for
         the lowlevel server function call handler:
...
def func_metadata(
    ...
    **critically** it also provides pre-parse helper to attempt to parmse things fro
    JSON.
    ...
    for param in params.values():
        ...
        if param.annotation is inspect.Parameter.empty:
            field_metadata.append(WithJsonSchema(
                {"title": param.name, "type": "string"}))
        ...
        if param.default is not inspect.Parameter.empty:
            dynamic_pydantic_model_params[field_name] = (
                Annotated[(annotation, *field_metadata,
                           Field(**field_kwargs))],
                param.default,
            )
        else:
            dynamic_pydantic_model_params[field_name] = Annotated[(
                annotation, *field_metadata, Field(**field_kwargs))]
    ...
    try:
        inspected_return_ann = inspect_annotation(
            sig.return_annotation, annotation_source=AnnotationSource.FUNCTION)
    except ForbiddenQualifier as e:
        raise InvalidSignature(
            f"Function {func.__name__}: return annotation contains an invalid type qualifier") from e
    ...
    if is_union_origin(get_origin(return_type_expr)):
        ...
        # Check if CallFunctionResult appears in the union (excluding None for Optional check)
        if any(isinstance(arg, type) and issubclass(arg, CallFunctionResult) for arg in args if arg is not type(None)):
            raise InvalidSignature(
                f"Function {func.__name__}: CallFunctionResult cannot be used in Union or Optional types. "
                "To return empty results, use: CallFunctionResult(content=[])"
            )

    original_annotation: Any
    # if the typehint is CallFunctionResult, the user either intends to return without validation
    # or they provided validation as Annotated metadata
    if isinstance(return_type_expr, type) and issubclass(return_type_expr, CallFunctionResult):
        if inspected_return_ann.metadata:
            ...
            if len(inspected_return_ann.metadata) >= 2:
                ...
                # i.e. from `Annotated[CallFunctionResult, ReturnType, Gt(1)]` to
                # `Annotated[ReturnType, Gt(1)]`:
                original_annotation = Annotated[
                    (return_type_expr, *inspected_return_ann.metadata[1:])
                ]  # pragma: no cover
            else:
                # We only had `Annotated[CallFunctionResult, ReturnType]`, treat the original annotation
...
def _try_create_model_and_schema(
    ...
    if model:
        ...
        try:
            ...
            logger.info(
                f"Cannot create schema for type {type_expr} in {func_name}: {type(e).__name__}: {e}")
...
def _create_model_from_typeddict(td_type: type[Any]) -> type[BaseModel]:
    ...
    required_keys = getattr(
        td_type, "__required_keys__", set(type_hints.keys()))
