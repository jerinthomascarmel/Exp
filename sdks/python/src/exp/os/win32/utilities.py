...
class FallbackProcess:
    ...
    def __init__(self, popen_obj: subprocess.Popen[bytes]):
        ...
        self.stdin = FileWriteStream(
            cast(BinaryIO, self.stdin_raw)) if self.stdin_raw else None
        self.stdout = FileReadStream(
            cast(BinaryIO, self.stdout_raw)) if self.stdout_raw else None
...
def _create_job_object() -> int | None:
    ...
    try:
        ...
        logger.warning(
            f"Failed to create Job Object for process tree management: {e}")
        return None
...
def _maybe_assign_process_to_job(process: Process | FallbackProcess, job: JobHandle | None) -> None:
    ...
    try:
        ...
        logger.warning(
            f"Failed to assign process {process.pid} to Job Object: {e}")
        if win32api:
